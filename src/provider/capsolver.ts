export const PROVIDER_ID = "capsolver";
import * as types from "../types/plugin";

import Debug from "debug";
const debug = Debug(`puppeteer-extra-plugin:recaptcha-capsolver:${PROVIDER_ID}`);

import capsolver_api from "./capsolver-api";

const solver = new capsolver_api();
...
async function decodeRecaptchaAsync(
	...
	return new Promise((resolve) => {
		...
		try {
			...
			let method = "ReCaptchaV2TaskProxyLess";
			if (vendor === "hcaptcha") {
